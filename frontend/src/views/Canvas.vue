...
const selectedFlow = computed(() =>
  flows.value.find((f) => f.id === currentFlowId.value) || null
)
...
async function loadFlowData(flowId) {
  ...
  error.value = ''
  try {
    const [flowRes, tablesRes, logicsRes] = await Promise.all([
      getFlow(flowId),
      getTables(),
      getLogics(),
    ])
    ...
    if (flowRes.success) {
      ...
      edges.value = (flow.edges || []).map((e) => ({
        ...
        label: e.label || '',
        animated: e.config?.animated ?? false,
      }))
    }
    ...
    nodes.value = [...defNodes, ...flowNodes]
  } catch (err) {
  ...
  <div class="canvas-page">
    <header class="canvas-header">
      <div class="canvas-header-left">
        ...
        <div v-if="showNewFlowInput" class="new-flow-form">
          ...
          <button
            ...
            @click="showNewFlowInput = false; newFlowName = ''"
          >
            Cancel
          </button>
        </div>
      ...
      <div class="canvas-header-right">
        ...
        <button class="btn-sm btn-secondary" @click="refreshDefinitions">⟳</button>
        <button
          class="btn-sm btn-primary"
          :disabled="!currentFlowId || saving"
          @click="handleSave"
        >
          {{ saving ? 'Saving...' : 'Save' }}
        </button>
